...
class Point_SERP(nn.Module):
    ...
    def forward(self, pts, gt_points, fake_labels, eval=False):
        ...
        x = torch.cat((patch_tokens, cls_tokens), dim=1) # B, G+1, 384
        pos = torch.cat((pos_embeds, cls_pos), dim=1) # B, G+1, 384

        x_encoder = self.serp_encoder(x, pos) # [B, G+1, 384]
        output_cls_token_reps = x_encoder[:, -1, :]
        
        x_encoder_no_cls = x_encoder[:, :-1, :] # [B, G, 384] 

        if eval:
            concat_f = torch.cat([output_cls_token_reps, x_encoder_no_cls.max(1)[0]], dim=-1)
            return concat_f

        decoder_pos = self.decoder_pos_embed(center) # [B, G, 384] 
        
        x_rec = self.serp_decoder(x_encoder_no_cls, decoder_pos) # [B, G, 384]
