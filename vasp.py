...
class VASP(nn.Module):
    ...
    def forward(self, pts, gt_points, fake_labels, eval=False):
        ...
        pos = torch.cat((encoder_pos, cls_pos), dim=1) # B, G+1, 384
        ...
        output_cls_token_reps = z_q[:, -1, :]
        z_q_no_cls = z_q[:, :-1, :] # [B, G, 384] 

        if eval:
            concat_f = torch.cat([output_cls_token_reps, z_q_no_cls.max(1)[0]], dim=-1)
            return concat_f

        decoder_pos = self.decoder_pos_embed(center)
        x_rec = self.serp_decoder(z_q_no_cls, decoder_pos) # [B, G, 384]
