...
def write_reconstructed(model, epoch, samples, img_folder, use_wandb = False):
    model.eval()
    roll_pitch = {"02691156": (90, 135), '04379243': (30, 30), '03642806': (30, -45), '03467517': (0, 90),
                  '03261776': (0, 75), '03001627': (30, -45)}
    ...
    for tax_id in label_ids.keys():
        ...
        condtn = tax_id[1:]
        model_df = pc_data.query(f'(synsetId == {condtn})').sample(n=samples, random_state = 1)
        # print(model_df.head())
        ...
        for idx, row in model_df.iterrows():
            ...
            plt.close()
            if use_wandb:
                wandb.log({tax_id: wandb.Image(path)})